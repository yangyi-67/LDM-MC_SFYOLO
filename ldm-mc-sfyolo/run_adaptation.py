...
def generate_dynamic_prompts(imgs_tensor):

    PROMPT_LEVEL_1 = "light fog, slight haze, clear details, realistic city street"
    PROMPT_LEVEL_2 = "foggy city street, mist, haze, realistic"
    PROMPT_LEVEL_3 = "dense fog, heavy haze, low visibility, whiteout, city street"

    with torch.no_grad():
        min_channel, _ = torch.min(imgs_tensor, dim=1) 
        density_scores = min_channel.view(imgs_tensor.size(0), -1).mean(dim=1)

    batch_prompts = []
    for score in density_scores:
        if score < 80: 
            batch_prompts.append(PROMPT_LEVEL_1)
        elif score < 150:
            batch_prompts.append(PROMPT_LEVEL_2)
        else:
            batch_prompts.append(PROMPT_LEVEL_3)
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    # Prefer dynamic check if available; fallback to True for safety on modern GPUs
    try:
        amp = check_amp(model_student)
    except Exception:
        amp = True
    ...
    adain = enhance_vgg16(opt) if opt.ta_method == 'tam' else None
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with torch.cuda.amp.autocast(amp):
                if opt.ta_method == 'tam':
                    imgs_style = get_style_images(imgs_255, opt, adain) / 255
                elif opt.ta_method == 'ldm':
                    dynamic_prompts_list = generate_dynamic_prompts(imgs_255)
                    opt_clone = deepcopy(opt) 
                    opt_clone.ldm_prompt = dynamic_prompts_list
                    imgs_style = get_ldm_images(imgs_255, opt_clone) / 255
                else:
                    imgs_style = imgs
                    
                # Teacher forward 
                model_teacher.eval()
                if opt.mc_dropout and opt.mc_T > 0:
                    handles = register_mc_dropout_hooks(model_teacher, p=opt.mc_p, only_detect=True)
                    preds_T = []
                    try:
                        for _t in range(opt.mc_T):
                            pt, _ = model_teacher(imgs)
                            preds_T.append(non_max_suppression(pt, conf_thres=conf_thres, iou_thres=iou_thres, multi_label=True, agnostic=single_cls, max_det=max_gt_boxes))
                    finally:
                        for h in handles:
                            h.remove()
                    min_votes = opt.mc_min_votes if opt.mc_min_votes is not None else max(2, int((opt.mc_T + 1) // 2))
                    pred_teacher_nms = aggregate_mc_predictions(preds_T, iou_match=opt.mc_iou_match, min_votes=min_votes, var_thr=opt.mc_var_thr)
                else:
...
def parse_opt(known=False):
    ...
    parser.add_argument('--hyp', type=str, default=ROOT / 'data/hyps/hyp.sfyolo.yaml', help='hyperparameters path')
    ...
    # Target augmentation method
    parser.add_argument('--ta_method', type=str, choices=['tam', 'ldm', 'none'], default='tam', help='Target augmentation method: TAM (style) or LDM (diffusion) or none')
    # TAM (style transfer) paths
    ...
    # LDM (diffusion) params
    parser.add_argument('--ldm_model', type=str, default='stabilityai/stable-diffusion-2-1', help='Diffusers model id for LDM img2img')
    parser.add_argument('--ldm_strength', type=float, default=0.35, help='LDM img2img strength [0,1]')
    parser.add_argument('--ldm_guidance_scale', type=float, default=5.0, help='Classifier-free guidance scale')
    parser.add_argument('--ldm_steps', type=int, default=25, help='Number of inference steps')
    parser.add_argument('--ldm_prob', type=float, default=1.0, help='Probability to apply LDM per image (0..1)')
    parser.add_argument('--ldm_cache_dir', type=str, default='', help='Optional cache dir to store/reuse LDM images')
    parser.add_argument('--ldm_prompt', type=str, default='foggy city street, dense fog, haze, realistic', help='Positive prompt for foggy style')
    parser.add_argument('--ldm_negative_prompt', type=str, default='clear sky, sunny, no fog, haze-free', help='Negative prompt')
    parser.add_argument('--ldm_seed', type=int, default=-1, help='Seed for diffusion (-1 for random)')
    # MC Dropout for pseudo-label uncertainty
    parser.add_argument('--mc_dropout', action='store_true', help='Enable MC Dropout for teacher pseudo labels')
    parser.add_argument('--mc_T', type=int, default=0, help='Number of MC passes (e.g., 5,10,15)')
    parser.add_argument('--mc_p', type=float, default=0.20, help='Dropout probability for MC Dropout hooks')
    parser.add_argument('--mc_iou_match', type=float, default=0.60, help='IoU threshold to match boxes across MC passes')
    parser.add_argument('--mc_min_votes', type=int, default=None, help='Minimum distinct-pass votes to accept a pseudo label (default ceil(T/2))')
    parser.add_argument('--mc_var_thr', type=float, default=0.04, help='Max variance of confidence across passes to accept box')
